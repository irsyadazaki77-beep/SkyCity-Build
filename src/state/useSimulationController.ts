            ...
            if (delta.changedTiles && delta.changedTiles.length > 0) {
              for (const update of delta.changedTiles) {
                if (newGrid[update.y]) {
                  ...
                  if (update.traffic !== undefined) t.traffic = update.traffic;
                  if (update.productivity !== undefined) t.productivity = update.productivity;
                  if (update.goodsStock !== undefined) t.goodsStock = update.goodsStock;
                  if (update.logisticsSatisfaction !== undefined) t.logisticsSatisfaction = update.logisticsSatisfaction;
                  if (update.commuteTime !== undefined) t.commuteTime = update.commuteTime;
                  if (update.landValue !== undefined) t.landValue = update.landValue;
                  if (update.pollution !== undefined) t.pollution = update.pollution;
                  if (update.noise !== undefined) t.noise = update.noise;
                  if (update.crime !== undefined) t.crime = update.crime;
